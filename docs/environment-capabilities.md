...
| بنر نصب PWA | وب — رفتار یکسان همه مرورگرها (عنوان/دکمه یکی؛ BIP یا مراحل) | خاموش | استفاده نمی‌شود |
| بنر آپدیت PWA | وب — رفتار یکسان؛ فقط سیاست محیط | خاموش | استفاده نمی‌شود |
| باز شدن لینک داخل PWA نصب‌شده | وب (`handle_links` + `launch_handler` در manifest) | خاموش / بی‌معنا | فقط کروم/اج روی اندروید و دسکتاپ؛ **iOS/Safari پشتیبانی نمی‌کند** |
